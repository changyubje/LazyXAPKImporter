...
public class XAPKDownloaderActivity extends Activity implements IDownloaderClient {
    ...
    public void onCreate(Bundle savedInstanceState)
    {
        ...
        String progressTitle = this.getIntent().getStringExtra("progressTitle");
        String progressMessage = this.getIntent().getStringExtra("progressMessage");

        Log.d(LOG_TAG, "Main? :"+mainVersion);
        Log.d(LOG_TAG, "Version :"+patchVersion+" FileSize : " + fileSize);
        ...
        if (!expansionFilesDelivered(mainVersion, patchVersion, fileSize)) {
            ...
            CallbackContext callbackContext = XAPKReader.lastCallbackContext;
            if(callbackContext != null){
                PluginResult result = new PluginResult(PluginResult.Status.OK, "Download Complete");
    ...
    public void onDownloadStateChanged(int newState) {
        ...
        switch (newState) {
            ...
            case STATE_COMPLETED: // The download was finished
                ...
                Log.v(LOG_TAG,"Download Success");
                {
                    CallbackContext callbackContext = XAPKReader.lastCallbackContext;
                    if(callbackContext != null){
                        PluginResult result = new PluginResult(PluginResult.Status.OK, "Download Complete");
            ...
            case STATE_FAILED:
                Log.v(LOG_TAG,"Download Fail "+newState);
                {

                    CallbackContext callbackContext = XAPKReader.lastCallbackContext;
                    if(callbackContext != null){
                        callbackContext.error("Download fail");
                    }
                }
